...
from satelib import get_logger, TIMING_LOG
import random, sys
                ...
                if self.return_code is None:
                    ...
                    try:
                        ...
                        if self._invocation[0].find('raxml') > -1:
                            self.error = RuntimeError("The reading results:\n'%s'\nfailed\nDue To: %s\nIt is a known issue that RAxML fails with large data set on 32-bit machine." % ("' '".join(self._invocation),errorFromFile))
                        else:
                            self.error = RuntimeError("The reading results:\n'%s'\nfailed \nDue To: %s" % ("' '".join(self._invocation),errorFromFile))
