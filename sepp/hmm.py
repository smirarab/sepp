...
def saveSequenceFileNames():
    ...
    np.save('./alignData/seqFileNames/names.npy', sequenceFileNames)
...
def giveSequenceFileNames():
    ...
    sequenceFileNames = np.load('./alignData/seqFileNames/names.npy')
...
def saveNewScores(strategyName):
    ...
    queryToHmm = np.load("./alignData/queryToHmm/original/" + strategyName + ".npy")
    ...
    for b in range(0, numHMM):
        ...
        hmmName = "./alignData/newHMM/hmm/" + strategyName + "_" + str(b) + ".hmm "
        #queryNames.append(queryNameO)
        queryNames.append(queryName)
        hmmNames.append(hmmName)


    scoreName = "./alignData/hmmScores/scoresFull/" + strategyName + "_full.npy"
...
def saveScoreFromBool(hmmNames, queryHMM, scoreName, noSave=False):
    ...
    for a in range(0, len(hmmNames)):
        ...
        if argsInclude.shape[0] > 0:
            ...
            queryNameTemp = './alignData/ML/temporaryStorage/1.fasta'
...
def saveScoresBySeq():
    ...
    for a in range(0, hmmNum):
        dataOld = np.load("./alignData/hmmScores/processed/" + str(a) + ".npy")
        for b in range(0, len(dataOld)):
            #dataOld[b, 0]
    ...
    np.save("./alignData/hmmScores/full.npy", data)
...
def saveDecomposition():
    ...
    sets = np.load("./alignData/hmmSets.npy", allow_pickle=True)
    ...
    np.save('./alignData/treeDecomp.npy', treeData)
...
def saveHMMsets():
    ...
    np.save("./alignData/hmmSets.npy", sets)
...
def reallignHMMSeq():
    ...
    for a in range(0, len(sequenceFileNames)):
        ...
        #if False:
        saveFastaBasic('./alignData/hmmSeqAlign/' + str(a) + '.fasta', nowKeys, nowSeqs)
...
def saveAdjustedScore():
    ...
    heightShift = np.log2(sizes1) * 2 #0.005 * np.max(scores)

    for a in range(len(scores[0])):
        scores[:, a] = scores[:, a] + heightShift[a]

    np.save("./alignData/hmmScores/fullAdjusted.npy", scores)
...
def scoresToHMMSeq(strategyName):
    ...
    if strategyName in ['stefan_UPP', 'stefan_UPPadjusted']:
        ...
        for a in range(0, len(HMMunique)):
            ...
            np.save('./alignData/newHMM/columnSets/'  + strategyName + "_" + str(a) + ".npy", usedCols)
            '''

            keys, seqs = loadFastaBasic(name)
            #seqs = removeEmptyColumns(seqs)
            saveFastaBasic("./alignData/newHMM/newHMMseq/" + strategyName + "_" + str(a) + ".fasta", keys, seqs)
    ...
    if strategyName == 'stefan_allwaysRoot':
        ...
        np.save("./alignData/queryToHmm/original/" + strategyName + ".npy", HMMroot)

        name = './alignData/hmmSeqAlign/' + str(0) + '.fasta'
        data = loadFastaFormat(name)
...
def resortToUPP(strategyName, doResort=True):
    ...
    np.save("./alignData/queryToHmm/withUPP/HMMused_" + strategyName + ".npy", HMMinverse)
    np.save("./alignData/queryToHmm/withUPP/UPPused_" + strategyName + ".npy", uppPosition)
    ...
    for a in range(len(uppToUseUnique)):
        ...
        keys, seqs = loadFastaBasic('./alignData/hmmSeqAlign/' + str(uppNum) + '.fasta')
        seqMatrix = []
        for seq in seqs:
            seqMatrix.append(list(seq))
...
def alignQueries(strategyName):
    ...
    for b in range(0, int(np.max(queryToHmm)) + 1):
        ...
        if argsHMM.shape[0] > 0:
            ...
            queryName1 = './alignData/hmmQueryList/inputQuery/' + strategyName + '_'  + str(b) + '.fasta'
            saveFasta(queryName1, dataNow)

            predictionName = './alignData/hmmQueryList/predictedQuery/' + strategyName + '_'  + str(b) + '.sto'

            if uppHMM[argsHMM[0]] == -1:
                hmmName = "./alignData/newHMM/hmm/" + strategyName + "_" + str(b) + ".hmm"
            else:
                uppNum = int(uppHMM[argsHMM[0]])
                hmmName = "./alignData/initialHMM/test" + str(uppNum) + ".hmm "
...
def scoreAlignment(strategyName):
    ...
    #trueFastaNew = './alignData/hmmQueryList/merged/' + 'stefan_TrueLimited' + '_TrueFasta.fasta'

    #a = 13



    #trueFasta = './alignData/trueAlign/true_align_fasta.fasta'
    #trueFasta = './alignData/trueAlign/true_align_fragged_fasta.fasta'
    #trueFastaNew = './alignData/hmmQueryList/merged/' + strategyName + '_TrueFasta.fasta'

    #trueFasta = './alignData/trueAlignment/original/R' + str(a) + '_true_align_fragged_fasta.fasta'
    #trueFasta = './alignData/trueAlignment/original/R' + str(a) + '_true_align_fragged_fasta.fasta'
    trueFasta = './alignData/trueAlignment/original/true_align_fragged_fasta.fasta'

    predictionName = './alignData/hmmQueryList/merged/' + strategyName + '_alignmentFasta.fasta'
    #predictionName = './alignData/hmmQueryList/merged/' + 'stefan_TrueUPP' + '_alignmentFasta.fasta'

    UPPName = './alignData/hmmQueryList/merged/' + 'stefan_TrueUPP' + '_alignmentFasta.fasta'

    trueFastaNew = './alignData/trueAlignment/subset/true_fasta.fasta'
    ...
    '''
    for a in range(len(predSeq)):
        seq1 = uppSeq[a].replace('-', '').upper()
        seq2 = predSeq[a].replace('-', '').upper()
        for b in range(0, len(seq1)):
            if seq1[b] != seq2[b]:
                print (seq1[b], seq2[b])
                quit()
        assert seq1 == seq2
    ...
    #print (len(trueKey))
    #a = 1
    #seq1 = trueSeq[a].replace('-', '')
    #seq2 = predSeq[a].replace('-', '')
    #seq1 = trueSeq[a]
    #seq2 = predSeq[a]
    #print (seq1)
    #print ('')
    ...
    trueSeqArray = []
    for seq in trueSeq:
        trueSeqArray.append(copy.copy(list(seq)))
    trueSeqArray = np.array(trueSeqArray).T
    trueSeqArray2 = []
    for a in range(0, len(trueSeqArray)):
        size1 = trueSeqArray[a][trueSeqArray[a] != '-'].shape[0]
        if size1 != 0:
            trueSeqArray2.append(np.copy(trueSeqArray[a]))
    trueSeqArray2 = np.array(trueSeqArray2).T
    trueSeq2 = []
    for a in range(0, len(trueSeqArray2)):
        str1 = ''.join(list(trueSeqArray2[a]))
        trueSeq2.append(str1)
    trueSeq2 = np.array(trueSeq2)

    trueSeq = np.copy(trueSeq2)
    quit()
    #'''
