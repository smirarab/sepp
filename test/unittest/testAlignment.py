...
class Test(unittest.TestCase):
    ...
    def testReadOnlySubAlignment(self):
        ...
        assert if readonly_subalignment.is_aligned() is True

        assert if readonly_subalignment.is_all_gap(2) is True,
        "Site 2 should be all gaps"
        assert if readonly_subalignment.is_all_gap(150) is False,
    ...
    def testExtendedAlignment(self):
        ...
        ("Columns are not correctly labeled after merging. Merged insertion "
         "count:%d. Insertion1:%d Insertion2:%d Mixed-insertion: %d") % (
