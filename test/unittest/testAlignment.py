...
class Test(unittest.TestCase):
    ...
    def testReadOnlySubAlignment(self):
        ...
        assert set(readonly_subalignment.keys()) == set(readonly_subalignment.get_sequence_names()) == set(subset), "Subalignment keys not matching given keys %s vs %s" %(list(readonly_subalignment.keys()) , subset)
        ...
        assert all([not mutable_subalignment.is_all_gap(i) for i in range(0,mutable_subalignment.get_length())])
    ...
    def testExtendedAlignment(self):
        ...
        cp1.fragments = ReadonlySubalignment([k for k in list(frg.keys()) if int(k[-1]) >= 9], frg)
        cp2.fragments = ReadonlySubalignment([k for k in list(frg.keys()) if int(k[-1]) <= 1], frg)
        ...
        assert ( in1 + in2 - mixed) == len(extmerger.get_insertion_columns()), "Columns are not correctly labeled after merging. Merged insertion count:%d. Insertion1:%d Insertion2:%d Mixed-insertion: %d"  %(len(list(extmerger.iter_insertion_columns())),in1 , in1, mixed)
        ...
        assert all([alg[k] == s for (k,s) in list(tmp.items())]), "merged alignment should match original alignment"
