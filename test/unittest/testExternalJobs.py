...
class TestExternalJob(ExternalSeppJob):
    ...
    def __init__(self, pipe = 0, **kwargs):
        ...
        if pipe == 0:
            ...
            output/err will be available at as self.stdoutdata and self.stderrdata
            and also will be logged'''
            pass           
        elif pipe == 1:
            ...
            1- A file object/descriptor cannot be used here because this is 
               executed in parent process, and in child process those file handlers
    ...
    def get_invocation(self):
        return ['find', self.pattern] + ([] if self.options is None else self.options.split())

    def characterize_input(self):
        return "pattern: %s options: %s (%s)" %(self.pattern, self.options, "pipe: %d" %self.pipe)

    def read_results(self):        
        return open("./.stdout").read() if self.pipe == 2 else self.stdoutdata if self.stdoutdata is not None else "Nothing captured" 
...
class Test(unittest.TestCase):
    ...
    def testError(self):            
        ...
        try:
            JobPool().enqueue_job(find_job)        
            assert JobPool().wait_for_all_jobs() == False            
        except JobError as e:
            assert str(e).find("No such file or directory") != -1, "The error we expected is no such file or directory"                 
        
        assert JobPool().get_asynch_result_object(find_job).successful() == False, "We expected the job to fail"
