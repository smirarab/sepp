...
class SearchJob(GenericJob):    
    def __init__(self, problem):
        ...
        Search job parent problems (if any) 
        '''
...
class ApplyModelJob(GenericJob):    
    def __init__(self, problem):
        '''
        Apply the model to fragments associated with each subproblem (equivalent of HMMERALIGN)
...
class Join_BuildModel_SearchFragment(Join):
    '''
    A join object used to join buildmodel jobs with their parent 
    ...
    def perform(self):
        ...
        4 - For each of grandchildrens of the grandparent problem, 
            ...
            4-2 Set the model attribute of their searchfragment job to the model
                computed in its buildmodel job
            4-3 Enqueue its searchfragment job
            
        The above procedure is equivalent of figuring out the child HMM model that
...
def build_job_dag(root_problem):
    ...
    for problem in root_problem.iter_nodes_at_level(SUMMERIZE_LEVEL):
        ...
        '''join these summarizejobs with all the leave applymodel jobs under them'''
